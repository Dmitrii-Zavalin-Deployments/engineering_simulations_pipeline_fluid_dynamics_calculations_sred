...
def test_main_cli_cpp_gate_stage(workspace_folder, monkeypatch):
    ...
    the entire sovereign SolverState container object is passed directly to the C++ engine constructor,
    ...
    # Setup mock C++ engine components to intercept constructor and step calls
    ...
    assert passed_state.x_min == 0.0
    assert passed_state.x_max == 1.0
    assert passed_state.y_min == 0.0
    assert passed_state.y_max == 1.0
    assert passed_state.z_min == 0.0
    assert passed_state.z_max == 1.0

    # 2.2 Tensors & Buffers
    assert passed_state.fields.shape == (4, 4, 4, 4)
    assert passed_state.mask.shape == (4, 4, 4)
    np.testing.assert_array_equal(passed_state.mask, np.zeros((4, 4, 4), dtype=np.int32))

    # 2.3 Simulation Timing & Iterations (Reflects post-run state after 3 completed time steps)
    assert passed_state.current_iteration == 3
    assert passed_state.current_time == 0.003
    assert passed_state.dt == 0.001
    assert passed_state.total_time == 0.003
    assert passed_state.total_iterations == 3
    assert passed_state.output_interval == 1

    # 2.4 Physical Constraints
    assert float(passed_state.physical_constraints["min_velocity"]) == -10.0
    assert float(passed_state.physical_constraints["max_velocity"]) == 10.0
    assert float(passed_state.physical_constraints["min_pressure"]) == -100.0
    assert float(passed_state.physical_constraints["max_pressure"]) == 100.0

    # 2.5 Domain Configuration
    assert passed_state.domain_configuration["type"] == "INTERNAL"
    assert passed_state.domain_configuration["reference_velocity"] == [0.0, 0.0, 0.0]

    # 2.6 Grid Configuration Dictionary
    assert passed_state.grid["nx"] == 4
    assert passed_state.grid["ny"] == 4
    assert passed_state.grid["nz"] == 4

    # 2.7 Fluid Properties
    assert float(passed_state.fluid_properties["density"]) == 1.0
    assert float(passed_state.fluid_properties["viscosity"]) == 0.01

    # 2.8 Initial Conditions
    assert passed_state.initial_conditions["velocity"] == [0.0, 0.0, 0.0]
    assert float(passed_state.initial_conditions["pressure"]) == 0.0

    # 2.9 Simulation Parameters Dictionary
    assert float(passed_state.simulation_parameters["time_step"]) == 0.001
    assert float(passed_state.simulation_parameters["total_time"]) == 0.003
    assert int(passed_state.simulation_parameters["output_interval"]) == 1

    # 2.10 Boundary Conditions
    assert len(passed_state.boundary_conditions) == 1
    assert passed_state.boundary_conditions[0]["location"] == "wall"
    assert passed_state.boundary_conditions[0]["type"] == "no-slip"

    # 2.11 External Forces
    assert passed_state.external_forces["force_vector"] == [0.0, 0.0, 0.0]
    assert passed_state.external_forces["gravity_vector"] == [0.0, -9.81, 0.0]

    # 2.12 Config Parameters
    assert int(passed_state.config["max_poisson_iterations"]) == 2000
    assert float(passed_state.config["poisson_tolerance"]) == 1e-8
