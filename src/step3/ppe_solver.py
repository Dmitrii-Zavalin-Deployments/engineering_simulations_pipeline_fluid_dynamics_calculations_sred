...
def solve_pressure_poisson_step(
  ...
  if not np.isfinite(p_old) or abs(p_old) > divergence_threshold:
    logger.error(
        ...
        f"--------------------------------------------------\n"
        f"  Val (p_old):           {p_old:.4e} (Threshold:"
        f" {divergence_threshold:.4e})\n"
        f"  Center Pressure (p_c): {p_c:.4e}\n"
        f"  Divergence (div_v*):   {div_v_star:.4e}\n"
        f"  Rhie-Chow Term:        {rhie_chow_term:.4e}\n"
        f"  RHS Source Term:       {rhs:.4e}\n"
        f"  Sum Neighbors:         {sum_neighbors:.4e}\n"
        f"  Stencil Denominator:   {stencil_denom:.4e}\n"
        f"  Grid Spacing (dx,dy,dz): {block.dx:.4e}, {block.dy:.4e},"
        f" {block.dz:.4e}\n"
        f"=================================================="
    )
