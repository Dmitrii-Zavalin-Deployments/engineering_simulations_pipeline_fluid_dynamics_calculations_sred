...
echo "📌 1. Rewriting $TARGET_FILE with strict heap allocation..."
...
TEST_F(MassContinuityIntegrationTest, EnforcesZeroDivergenceInFluidDomain) {
    ...
    // Heap-allocate orchestrator via std::unique_ptr to prevent stack overflow
    std::unique_ptr<NavierStokesOrchestrator> orchestrator = 
        std::make_unique<NavierStokesOrchestrator>(dims_, config_);
...
echo "📌 2. Purging build cache for a fresh compilation..."
rm -rf build
...
echo "📌 5. Locating binary and running under AddressSanitizer..."
